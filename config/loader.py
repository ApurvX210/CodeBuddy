...
def get_config_dir() -> Path:
    return Path(user_config_dir('CodeBuddy'))

def get_system_config_path() -> Path:
    return get_config_dir() / CONFIG_FILE_NAME

def _get_project_configuration(cwd: Path) -> Path | None:
    current = cwd.resolve()
...
def _parse_toml(path: Path):
    try:
        ...
        raise ConfigError(f"Failed to read TOML in {path}:{e}",config_file=str(path)) from e
...
def load_config(cwd: Path | None) -> Config:
    ...
    project_path = _get_project_configuration(cwd)
    if project_path:
        try:
    ...
    if "developer_instruction" not in config_dict:
        agent_md_content = _get_agent_md_file(cwd)
        if agent_md_content:
            config_dict["developer_instruction"] = agent_md_content
    try:
